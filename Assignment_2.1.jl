...
# ╔═╡ 7712782d-bdde-46d4-9e05-d3e6c8b3c3b8
using PlutoUI, LinearAlgebra,  DifferentialEquations

# ╔═╡ 6eff15b6-625c-4d77-a93f-66c346b891ff
md"""
# 2
"""

# ╔═╡ b01e6d74-fa23-11ee-3c72-c90c5ecc732a
begin
	...
	t = t₀ +(100*24*60*60) # time + 100days [s] orginally had t=(t₀ + 100)*(24*60*60)
...
# ╔═╡ 28cf3336-441e-49cc-9be6-323be65b7fe8
md"""
### 2.1.1
The equations of motion for the two-body problem between the Earth and an object with neglible mass are:

$\ddot{\vec{r}} = -\frac{\mu}{r^3} \vec{r}$ 

where $\vec{r}$ is the relative postion vector of the object. This is a second order differential equation which governs the motion of the first mass relative to the second. It has two vector constants of integration, each of these containing three scalar components. Therefore there are six constants of integration in total. In the co-moving reference frame (the frame attached to the larger mass, in this case Earth), $\vec{r}$ has the expression:

$\vec{r} = x\hat{i} + y \hat{j} + z\hat{k}$

The relative accelleration components in this moving frame of reference are then:

$\ddot{x} = - \frac{\mu}{r^3} x$
$\ddot{y} = - \frac{\mu}{r^3} y$
$\ddot{z} = - \frac{\mu}{r^3} z$

Defining the the variables $u, v, w$ as the derivatives of the state vector components $x, y, z$, the system of equations may be reduced to a system of first order differential equations:
...
# ╔═╡ a70093fb-4981-4236-a9bc-106acecc225f
md"""
### 2.1.2
A function is created which outputs the right hand side of the above system of first-order ordinary differential equations.
"""

# ╔═╡ e1249240-dca7-4f3d-8b31-1acbcb9c3455
function TBP_ECI!(ds,s,p,t)
...
# ╔═╡ 1557fbc9-c7b6-4bdc-acc0-fa20b4612748
md""" ### 2.1.3 
A set of initial conditions in the ECI reference frame are stored in the variable $s_i$. Using these initial conditions, the equations of motion may be integrated using the Julia package DifferentialEquations.jl with the Tsitouras-Papakostas 8/7 Runge-Kutta method for a total of 2 orbital periods of the object. The state vector is saved at time intervals of 10 seconds.

"""

# ╔═╡ 5841ec33-20b0-40ce-842f-bc59835efb61
begin
	# Initial State Vector
	sᵢ = [−3174.89878864632, −992.843201535607, −6922.86051061034, 5.29933974840672, 3.41395143650654, −2.9199470061499]
	
	ti = 0.0 # Initial time
	tf1 = 6050*2 # Final time
	alg = TsitPap8() # Algorithm definition

# Define the problem: equations, initial condition, timespan, parameters
	p = μₑ
	prob1 = ODEProblem(TBP_ECI!, sᵢ, (ti, tf1), p);
	sol1 = solve(prob1, alg, reltol=1e-12, abstol=1e-12, saveat = ti:10:tf1)

	# Pull out data from the solution
	x= sol1[1,:];
	y= sol1[2,:];
	z= sol1[3,:];
	u= sol1[4,:];
...
# ╔═╡ 44fe943f-356b-4928-8191-b3dffaaab216
begin
using GeometryBasics, CairoMakie, FileIO

	img=load(download("https://svs.gsfc.nasa.gov/vis/a000000/a002900/a002915/bluemarble-2048.png"));
		limit = 2*Rₑ
		
		f = Figure()
		aa = Axis3(f[1,1], aspect = (1,1,1), viewmode = :fit,
		   limits = (-limit/2, limit/2, -limit/2, limit/2, -limit/2, limit/2),  title = "Trajectory in ECI Reference Frame", xlabel = "x [km]", ylabel = "y [km]", zlabel = "z [km]")
		
		earth = GeometryBasics.Sphere(Point3f(0), Rₑ)
		mesh!(aa, earth, color=img)

	lines!(x, y, z, color = :red)
	f
end 

# ╔═╡ 899406b3-b505-4bef-ac88-e73d8cea52e6
md"""
The output of integrating the equations of motion for 2 periods is plotted as a 3-D trajectory in the ECI reference frame including a spherical representation of the Earth.
"""

# ╔═╡ 916d109e-cf08-45b4-bbd5-b6c505fd8786
begin
	k = ((norm.(eachrow(hcat(u, v, w)))).^2)./2 # Specific Kinetic Energy

	U = -μₑ./(norm.(eachrow(hcat(x, y, z)))) # Specific Potential Energy

	ϵ = k + U # Specific total mechanical Energy

	tf = (ti/60^2:10/60^2:tf1/60^2) # Time span [hrs]
	
fig1 = Figure()
	ax = Axis(fig1[1, 1], xlabel = "Time [hrs]", ylabel = "Energy [km²/s²]") 
lines!(ax, tf, k, label = "k")
	lines!(ax, tf, U, label = "U")
	lines!(ax, tf, ϵ, label = "ϵ")
	axislegend("Legend", position = :rc, orientation = :horizontal)
fig1
end

# ╔═╡ fbedeca1-d6f2-4b33-a08f-ac51a8338741
md"""
### 2.1.4 

Specific Energies may be calculated as follows for the preceding object orbiting the Earth:

- The Relative Kinetic Energy per unit mass is given by $k = \frac{V^2}{2}$, where $V$ is the magnitude of the velocity vector $V = \|\vec{V}\| = \sqrt{u^2 + v^2 + w^2}$.
- The Potential Energy per unit mass of the orbiting body in the gravitational field of the earth is given by $u = -\frac{\mu}{r}$, where $r$ is the magnitude of the position vector $r = \|\vec{r}\| = \sqrt{x^2 + y^2 + z^2}$.
- The total mechanical energy per unit mass is the sum of the kinetic and potential energies per unit mass, and is given by $\epsilon = k + u = \frac{V^2}{2} -\frac{\mu}{r}$.

These are plotted as a function of time for the orbit as it completes 2 revolutions. The kinetic energy oscillates periodically in sync with the changing velocity of the spacecraft. It is maximum when the spacecraft is at perigee and minimum when at apogee. There is not a huge varation in the kinetic energy because the eccentricity of the orbit is low - thus the velocity remains fairly constant over the orbit.

The potential energy is inversely proportional to the magnitude of the postion vector (i.e. the distance from the Earth). As such, it is maximum at apogee and minimum and perigee and is 180° out of phase with the kinetic energy.

The sum of the kinetic and potential energies represents the specific orbital energy. For conservation of energy, this should remain constant over the orbit. It is observed that this is indeed true as the specific orbital energy is constant at $(ϵ[1]) [km²/s²].

Moreover, because the specific orbital energy is $\epsilon < 0$, the motion is bounded and oscillates between a minimum and a maximum value of $r$.
"""

# ╔═╡ 015152bd-eb2a-4e0e-9a3e-cbe3c147bb77
begin
	h = norm.(cross.(eachrow(hcat(x, y, z)), eachrow(hcat(u, v, w)))) # magnitude of the Specific Angular Momentum

	fig2 = Figure()
	ax2 = Axis(fig2[1, 1], xlabel = "Time [hrs]", ylabel = "Magnitude of Angular Momentum [km²/s]")
	ylims!(ax2, 53340, 53370)
lines!(ax2, tf, h)
fig2
	
end

# ╔═╡ adb302a0-272c-4ae7-a414-bb2063046780
md"""
The Specific Angular Momentum ($\vec{h}$) is given by the cross product between $\vec{r}$ and $\vec{V}$, where $\vec{r}$ and $\vec{V}$ are the position and velocity vectors respectively:

$\vec{h} = \vec{r} \times \vec{V}$

This quantity is an integral of motion in Keplerian dynamics and according to Keplerian dynamics, should be constant in time:

$\frac{d \vec{h}}{d t} = 0$

Plotting the magnitude of $\vec{h}$ over the time for 2 orbits reveals that it is indeed constant in time for this duration at $(h[1]) [km²/s].
"""
...
manifest_format = "2.0"
project_hash = "df86ce7f772a26ca2ee0bc496e59965762c30c63"
...
[[deps.NonlinearSolve]]
deps = ["ADTypes", "ArrayInterface", "ConcreteStructs", "DiffEqBase", "DifferentiationInterface", "EnumX", "FastBroadcast", "FastClosures", "FiniteDiff", "ForwardDiff", "LazyArrays", "LineSearch", "LineSearches", "LinearAlgebra", "LinearSolve", "Markdown", "MaybeInplace", "PrecompileTools", "Preferences", "Printf", "RecursiveArrayTools", "Reexport", "SciMLBase", "SciMLJacobianOperators", "SciMLOperators", "Setfield", "SimpleNonlinearSolve", "SparseArrays", "SparseConnectivityTracer", "SparseMatrixColorings", "StaticArraysCore", "SymbolicIndexingInterface", "TimerOutputs"]
git-tree-sha1 = "1c46688b47f732dde6ccb5fe37a435745ef36042"
...
deps = ["ADTypes", "ArrayInterface", "ConcreteStructs", "DiffEqBase", "DiffResults", "DifferentiationInterface", "EnumX", "FastClosures", "FiniteDiff", "ForwardDiff", "LinearAlgebra", "Markdown", "MaybeInplace", "PrecompileTools", "Reexport", "SciMLBase", "Setfield", "StaticArraysCore"]
git-tree-sha1 = "19987527cf5b78c97637e1384c47e52ca5023c82"
